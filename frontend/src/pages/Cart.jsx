...
const Cart = () => {
    const {
        products, cartItems, addToCart, removeFromCart, deleteFromCart,
        getCartTotalAmount, currency, delivery_fee
    } = useContext(ShopContext);

    const navigate = useNavigate();
    const cartTotal = getCartTotalAmount();

    const userCart = products.filter(item => cartItems[item._id] > 0);

    return (
        <div className='bg-stone-50 min-h-[calc(100vh-150px)]'>
            <div className='max-w-6xl mx-auto px-4 py-16 lg:py-24'>
                <h1 className='text-3xl font-bold text-center mb-12 text-gray-800'>Shopping Cart</h1>

                {cartTotal === 0 ? (
                    <div className='text-center py-20 bg-white border rounded-lg shadow-sm'>
                        <h2 className='text-2xl font-semibold text-gray-700'>Your Cart is Empty</h2>
                        <button onClick={() => navigate('/collection')} className='mt-6 bg-black text-white py-3 px-8 rounded-md hover:bg-gray-800 transition'>
                            Continue Shopping
                        </button>
                    </div>
                ) : (
